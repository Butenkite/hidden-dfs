...
public class Traverse {
  ...
  public static boolean reachable(Map<Integer, Set<Integer>> graph, int start, int end){
    ...
    return reachableHelper(graph, start, end, visited);
  }

  public static boolean reachableHelper(Map<Integer, Set<Integer>> graph, int start, int end, Set<Integer> visited){
    if(start == end) return true;
    if(visited.contains(start) || graph == null || !graph.containsKey(start) || graph.get(start) == null) return false;
    visited.add(start);
    for(int neighbor : graph.get(start)){
      if(reachableHelper(graph, neighbor, end, visited)) return true;
    }
    return false;
  }
}
